...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (apiDataResp) {
        setApiData(
          Object.entries(apiDataResp).map(([value, label]) => ({
            value,
            label,
          })),
        );
      }
      const curDefault = await fetchCurrencyData(
        `latest/v1/currencies/${fromCurrency}.json`,
      );
      if (curDefault) {
  ...
  const fetchHistory = async (currency) => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const dayData = await fetchCurrencyData(
        `${formattedDate}/v1/currencies/${currency}.json`,
      );
      if (dayData) {
            ...
            {rates && (
              <div className='rate_container toast '>
                ...
                {POPULAR_CURRENCIES.map((item) => {
                  return (
                    <div key={item} className='pop-rate-item'>
                      {item.toUpperCase()}: {amount * Number(rates[item] || 0)}
                    </div>
                  );
                })}
              </div>
